...
test-unit:
	docker run --name unit-tests --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest -c "pytest --cov --cov-report=xml:results/coverage.xml --cov-report=html:results/coverage --junit-xml=results/unit_result.xml -m unit && junit2html results/unit_result.xml results/unit_result.html" || true
...
test-api:
	...
	docker run --network calc-test-api --name api-tests --env PYTHONPATH=/opt/calc --env BASE_URL=http://apiserver:5000/ -w /opt/calc calculator-app:latest -c "pytest --junit-xml=results/api_result.xml -m api &&  junit2html results/api_result.xml results/api_result.html" || true
...
test-e2e:
	...
	docker create --network calc-test-e2e --name e2e-tests -c" cypress/included:4.9.0 --browser chrome && junit2html results/cypress_result.xml results/cypress_result.html" || true
